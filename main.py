...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
	...
	# 1x1 conv layer4, 10x36xnum_classes
    layer4_conv = tf.layers.conv2d(vgg_layer4_out, num_classes, 1, padding = 'same', 
                                   kernel_initializer = tf.random_normal_initializer(stddev = 0.01), 
                                   kernel_regularizer = tf.contrib.layers.l2_regularizer(1e-3))
    # skip connection (element-wise addition)
    skip_layer1 = tf.add(decoder_layer1, layer4_conv)
    # upsample, Decoder layer 2
    decoder_layer2 = tf.layers.conv2d_transpose(skip_layer1, num_classes, 4, strides = (2, 2), padding = 'same', 
                                             kernel_initializer = tf.random_normal_initializer(stddev = 0.01), 
                                             kernel_regularizer = tf.contrib.layers.l2_regularizer(1e-3))
	# 1x1 conv layer3 
    layer3_conv = tf.layers.conv2d(vgg_layer3_out, num_classes, 1, padding = 'same', 
                                   ...
                                   kernel_regularizer = tf.contrib.layers.l2_regularizer(1e-3))
    # skip connection
    skip_layer2 = tf.add(decoder_layer2, layer3_conv)
    # upsample, Decoder layer 3
    nn_last_layer = tf.layers.conv2d_transpose(skip_layer2, num_classes, 16, strides= (8, 8), padding = 'same', 
                                               kernel_initializer = tf.random_normal_initializer(stddev = 0.01), 
                                               kernel_regularizer = tf.contrib.layers.l2_regularizer(1e-3))
...
def run():
    num_classes = 2
    image_shape = (160, 576)
    data_dir = '/data'  ##train in Udacity workspace.
    runs_dir = './runs'
    ...
    with tf.Session() as sess:
        ...
        # set placeholders for labels and learning_rate
